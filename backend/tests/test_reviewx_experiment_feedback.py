...
from app.modules.review.reviews_api import (
    ...
    list_reviewx_experiment_runs_endpoint,
...
def test_optimization_policy_accepts_primary_gain_when_guardrails_hold():
    dossier, assessment, evidence = _models()
    previous_payload = evidence.model_dump(mode="json")
    for metric in previous_payload["metrics"]:
        if metric["name"] == "unsupported_claim_rate":
        ...
        if metric["name"] == "citation_precision":
            metric["value"] = 0.86
    previous = ExperimentEvidence.model_validate(previous_payload)
    policy = ExperimentOptimizationPolicy(
        primaryMetric="unsupported_claim_rate",
        direction="minimize",
        minimumImprovement=0.05,
        guardrails=[
            MetricGuardrail(name="citation_precision", direction="maximize", threshold=0.85),
        ],
    )
    ...
    assert result.iterationDecision.decision == "accept_results"
    assert result.iterationDecision.primaryObjective.improvement == 0.08
    assert result.iterationDecision.primaryObjective.satisfied is True
    assert result.iterationDecision.guardrailViolations == []
    assert result.iterationDecision.benchmarkComparable is True


def test_optimization_policy_blocks_single_metric_gain_that_breaks_guardrail():
    dossier, assessment, evidence = _models()
    current_payload = evidence.model_dump(mode="json")
    for metric in current_payload["metrics"]:
        if metric["name"] == "citation_precision":
            metric["value"] = 0.7
    current = ExperimentEvidence.model_validate(current_payload)
    previous_payload = evidence.model_dump(mode="json")
    for metric in previous_payload["metrics"]:
        if metric["name"] == "unsupported_claim_rate":
            metric["value"] = 0.2
    ...
    result = review_experiment_feedback(
        ...
        execution_assessment=assessment,
        previous_experiment=previous,
        optimization_policy=ExperimentOptimizationPolicy(
            primaryMetric="unsupported_claim_rate",
            direction="minimize",
            minimumImprovement=0.05,
            guardrails=[
    ...
    assert result.iterationDecision.primaryObjective.satisfied is True
    assert result.iterationDecision.decision == "revise_plan"
    assert result.iterationDecision.guardrailViolations == ["citation_precision"]
    assert "EXPERIMENT_GUARDRAIL_VIOLATED" in {
        finding.code for finding in result.qualityAssessment.findings
    }


def test_changed_benchmark_blocks_cross_iteration_metric_comparison():
    dossier, assessment, evidence = _models()
    previous_payload = evidence.model_dump(mode="json")
    previous_payload["dataHashes"] = {"dataset.csv": "different-hash"}
    previous = ExperimentEvidence.model_validate(previous_payload)
    ...
    assert result.qualityAssessment.gateStatus == "fail"
    assert result.iterationDecision.decision == "revise_plan"
    assert result.iterationDecision.metricDeltas == []
    assert result.iterationDecision.benchmarkComparable is False
...
def test_stored_feedback_history_survives_refresh(monkeypatch, tmp_path):
    ...
    assert repeated.feedbackId == response.feedbackId
    assert history["total"] == 1
    assert history["records"][0]["id"] == response.feedbackId
    assert history["records"][0]["iterationDecision"]["decision"] == "accept_results"


def test_faros_runtime_runs_are_listed_and_audited_without_legacy_storage(monkeypatch):
    dossier, assessment, evidence = _models()
    faros_run_id = "faros_runtime_001"
    ...
    class FakeOrchestrator:
        def list_runs(self):
            return [{
                ...
                "profile_id": "faros_qwen",
                "inputs": {"seedQuery": dossier.problemFrame.originalQuestion},
            }]

        def get_run(self, run_id):
            return self.list_runs()[0] if run_id == faros_run_id else None

        def get_run_memory(self, run_id):
            if run_id != faros_run_id:
                return None
            return {"data": {
                "seedQuery": dossier.problemFrame.originalQuestion,
                "experimentDesign": {
                    "title": "Runtime experiment",
                    "objective": dossier.researchPlan.objective,
                    "hypothesis": dossier.hypotheses[0].statement,
                },
        ...
        def list_artifacts(self, run_id):
            return [
                {"id": f"{run_id}:assessment", "type": "execution_assessment"},
                {"id": f"{run_id}:evidence", "type": "experiment_evidence"},
            ]
    ...
    listing = asyncio.run(list_reviewx_experiment_runs_endpoint())
    response = asyncio.run(
        run_stored_experiment_feedback_endpoint(
            faros_run_id,
            RunStoredExperimentFeedbackRequest(),
    ...
    assert listing["runs"][0]["id"] == faros_run_id
    assert response.runId == faros_run_id
    assert response.sourceArtifacts["experimentEvidence"] == f"{faros_run_id}:evidence"
    assert response.iterationDecision.decision == "accept_results"
