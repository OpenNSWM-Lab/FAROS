...
export function CompetitionEvidence() {
  ...
  const pageActions = (
    <div className="flex flex-wrap items-center gap-2">
      ...
      >
        ...
        <span className="ml-2">{text('真实 Qwen 复验', 'Run with Qwen')}</span>
      </Button>
    </div>
  )
  ...
  return (
    <AppPageLayout
      title={text('ReviewX · 方向 1B 证据驾驶舱', 'ReviewX · Track 1B Evidence Dashboard')}
      subtitle={text('FAROS 科学实验任务规划与反馈迭代的可核验结果', 'Verifiable planning, feedback, and iteration evidence from FAROS')}
      icon={Gauge}
      iconColor="blue"
      accentColor="blue"
      actions={pageActions}
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs leading-relaxed text-sky-900">
              {text(
                '本页会自动载入服务器上已经完成的 benchmark 和 Qwen 复验快照，供比赛展示与复核；它们不是当前账号进入页面后新生成的数据。只有点击“真实 Qwen 复验”才会创建新任务。',
                'This page automatically loads completed benchmark and Qwen verification snapshots from the server for demonstration and inspection. They were not generated by simply opening this page; only “Run with Qwen” creates a new job.',
              )}
            </p>
          </div>
        </div>
